...
class SolutionCreateView(UserPassesTestMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["attachment_form"] = self.attachment_form_class(prefix="attachment_form")
        return context
    ...
    def save_solution_attachment(self, form):
    ...
    def post(self, request, *args, **kwargs):
        ...
        solution_attachment_form = self.attachment_form_class(request.POST, request.FILES, prefix="attachment_form")
        if form.is_valid():
            self.form_valid(form)
            try:
                self.save_solution_attachment(solution_attachment_form)
            except KeyError:
                pass
            return HttpResponseRedirect(self.get_success_url())
        else:
